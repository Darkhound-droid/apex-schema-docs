...
CREATE OR REPLACE PACKAGE BODY apex_schema_docs_pkg AS
  ...
  FUNCTION get_view_column_count(
  ...
  BEGIN
    BEGIN
      EXECUTE IMMEDIATE
        'SELECT COUNT(*) FROM user_view_columns WHERE view_name = :1'
      INTO l_count
      USING UPPER(p_view_name);
    EXCEPTION
      WHEN OTHERS THEN
    ...
    RETURN l_count;
  END get_view_column_count;

  FUNCTION get_view_columns_csv(
    p_view_name IN VARCHAR2
  ) RETURN VARCHAR2 IS
  ...
  BEGIN
    ...
    RETURN l_columns;
  END get_view_columns_csv;

  PROCEDURE open_view_columns_cursor(
    p_view_name IN VARCHAR2,
  ...
  BEGIN
    ...
    FOR view_rec IN (
      ...
      BEGIN
        ...
        l_columns := get_view_columns_csv(view_rec.view_name);
    ...
    IF has_selected_names(p_view_names) THEN
      FOR view_rec IN (
        ...
        BEGIN
          ...
          BEGIN
            open_view_columns_cursor(view_rec.view_name, l_view_columns);
            LOOP
              FETCH l_view_columns INTO l_column_name;
              EXIT WHEN l_view_columns%NOTFOUND;

              IF NOT l_first_column THEN
                append_text(l_output, ',' || g_newline);
              END IF;
              l_first_column := FALSE;
              append_line(l_output, '        "' || escape_json(l_column_name) || '"');
            END LOOP;
            CLOSE l_view_columns;
          EXCEPTION
            WHEN OTHERS THEN
              IF l_view_columns%ISOPEN THEN
                CLOSE l_view_columns;
              END IF;
              RAISE;
          END;
